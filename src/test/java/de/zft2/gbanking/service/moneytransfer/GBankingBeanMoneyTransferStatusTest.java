...
class GBankingBeanMoneyTransferStatusTest {
	...
	void saveMoneyTransferToDB_shouldMarkEditedInventoryTransferAsChanged() {
		...
		Recipient recipient = dbController.insertOrUpdate(new Recipient("Inventory Recipient", "DE11111111111111111111", "TESTDEFFXXX", null, null,
				"Testbank", Source.MONEYTRANSFER));
		...
		MoneyTransfer savedTransfer = bean.saveMoneyTransferToDB(changedForm, inventoryTransfer);
		...
		assertEquals("Changed purpose", transfers.stream().filter(transfer -> transfer.getId() == savedTransfer.getId()).findFirst().orElseThrow().getPurpose());
	...
	void saveMoneyTransferToDB_shouldKeepNewStatusForNewTransfer() {
		...
		MoneyTransfer savedTransfer = bean.saveMoneyTransferToDB(form);
	...
	void retrieveOpenTransfers_shouldReturnExecutableTransfers() {
		BankAccount account = dbController.insertOrUpdate(TestData.createSampleAccount(null));
		Recipient recipient = dbController.insertOrUpdate(new Recipient("Open Transfer Recipient", "DE33333333333333333333", "TESTDEFFXXX", null,
				null, "Testbank", Source.MONEYTRANSFER));
		dbController.insertOrUpdate(new MoneyTransfer(account.getId(), OrderType.TRANSFER, recipient.getId(), "Open transfer",
				new BigDecimal("29.99"), LocalDate.of(2026, Month.AUGUST, 17), MoneyTransferStatus.NEW));
		dbController.insertOrUpdate(new MoneyTransfer(account.getId(), OrderType.TRANSFER, recipient.getId(), "Changed transfer",
				new BigDecimal("34.99"), LocalDate.of(2026, Month.AUGUST, 18), MoneyTransferStatus.CHANGED));
		dbController.insertOrUpdate(new MoneyTransfer(account.getId(), OrderType.TRANSFER, recipient.getId(), "Retry transfer",
				new BigDecimal("36.99"), LocalDate.of(2026, Month.AUGUST, 19), MoneyTransferStatus.ERROR));
		...
		dbController.insertOrUpdate(new MoneyTransfer(account.getId(), OrderType.TRANSFER, recipient.getId(), "Sent transfer",
				new BigDecimal("39.99"), LocalDate.of(2026, Month.AUGUST, 18), MoneyTransferStatus.SENT));

		List<MoneyTransfer> transfers = bean.retrieveOpenTransfers();
	...
	void getAllByParentWithFilter_shouldApplyParentAndEnumFilter() {
		...
		Recipient recipient = dbController.insertOrUpdate(new Recipient("Parent Filter Recipient", "DE44444444444444444444", "TESTDEFFXXX", null,
				null, "Testbank", Source.MONEYTRANSFER));
		dbController.insertOrUpdate(new MoneyTransfer(account.getId(), OrderType.TRANSFER, recipient.getId(), "Matching transfer",
				new BigDecimal("49.99"), LocalDate.of(2026, Month.AUGUST, 19), MoneyTransferStatus.NEW));
		dbController.insertOrUpdate(new MoneyTransfer(account.getId(), OrderType.TRANSFER, recipient.getId(), "Wrong status",
				new BigDecimal("59.99"), LocalDate.of(2026, Month.AUGUST, 20), MoneyTransferStatus.SENT));
		dbController.insertOrUpdate(new MoneyTransfer(otherAccount.getId(), OrderType.TRANSFER, recipient.getId(), "Wrong account",
				new BigDecimal("69.99"), LocalDate.of(2026, Month.AUGUST, 21), MoneyTransferStatus.NEW));
	...
	void requestAndCancelBankOrderDeletion_shouldKeepOrderRetryable() {
		BankAccount account = dbController.insertOrUpdate(TestData.createSampleAccount(null));
		Recipient recipient = dbController.insertOrUpdate(new Recipient("Deletion Recipient", "DE55555555555555555555", "TESTDEFFXXX", null, null,
				"Testbank", Source.MONEYTRANSFER));
		MoneyTransfer inventoryTransfer = createInventoryTransfer(account.getId(), recipient.getId());
		inventoryTransfer.setBankOrderId("scheduled-delete-1");
		inventoryTransfer = dbController.insertOrUpdate(inventoryTransfer);

		MoneyTransfer deletionPending = bean.requestBankOrderDeletion(inventoryTransfer);

		assertEquals(MoneyTransferStatus.DELETE_PENDING, deletionPending.getMoneytransferStatus());
		assertEquals(deletionPending.getId(), bean.retrieveOpenTransfers().get(0).getId());

		MoneyTransfer restored = bean.cancelBankOrderDeletion(deletionPending);

		assertEquals(MoneyTransferStatus.INVENTORY, restored.getMoneytransferStatus());
		assertEquals(0, bean.retrieveOpenTransfers().size());
	}

	@Test
	void requestBankOrderDeletion_shouldDiscardPendingEditAndDeleteConfirmedVersion() {
		BankAccount account = dbController.insertOrUpdate(TestData.createSampleAccount(null));
		Recipient recipient = dbController.insertOrUpdate(new Recipient("Changed Recipient", "DE66666666666666666666", "TESTDEFFXXX", null, null,
				"Testbank", Source.MONEYTRANSFER));
		MoneyTransfer inventoryTransfer = createInventoryTransfer(account.getId(), recipient.getId());
		...
		MoneyTransfer changedTransfer = bean.saveMoneyTransferToDB(changedForm, inventoryTransfer);

		MoneyTransfer deletionPending = bean.requestBankOrderDeletion(changedTransfer);
