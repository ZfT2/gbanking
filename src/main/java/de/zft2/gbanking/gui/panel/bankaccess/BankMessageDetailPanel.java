...
final class BankMessageDetailPanel extends AbstractReadonlyDetailPanel {
	...
	BankMessageDetailPanel(Runnable retrieveAction) {
	...
	void updateBankAccess(BankAccess bankAccess, boolean supported) {
	...
	void updateMessage(BankMessage bankMessage) {
		if (bankMessage == null) {
			clearMessage();
			return;
		}
