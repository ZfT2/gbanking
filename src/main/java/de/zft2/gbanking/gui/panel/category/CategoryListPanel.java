...
public class CategoryListPanel extends AbstractFilterableTablePanel<Category> {
	...
	private void createInnerCategoryListPanel() {
		...
		configureTableLayout("categories");
		onSelection(parentPanel.getCategoryInputPanel()::updatePanelFieldValues);
		reload();
	}

	private List<TableColumn<Category, ?>> createColumns() {
		TableColumn<Category, Boolean> selectedCol = createSelectAllSelectionColumn(
				category -> category.isSelected(), (category, selected) -> category.setSelected(selected));
	...
	protected boolean matchesFilter(Category category, String filter) {
		return matchesAny(filter, category.getName(), category.getFullName());
	}

	public void reload() {
		replaceItemsFrom(() -> dbController.getAll(Category.class));
	}
