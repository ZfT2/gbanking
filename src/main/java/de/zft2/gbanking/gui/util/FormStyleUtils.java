...
public final class FormStyleUtils {
	...
	private static final String GBANKING_READONLY_CONTROL = "gbanking-readonly-control";
	...
	public static void setReadOnlyStyle(boolean readOnly, Control... controls) {
		for (Control control : controls) {
			if (readOnly) {
				if (!control.getStyleClass().contains(GBANKING_READONLY_CONTROL)) {
					control.getStyleClass().add(GBANKING_READONLY_CONTROL);
				}
			} else {
				control.getStyleClass().remove(GBANKING_READONLY_CONTROL);
			}
		}
	}
