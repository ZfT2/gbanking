...
public class TenantBackupService {
	...
	private static final Logger log = LogManager.getLogger(TenantBackupService.class);
	...
	public TenantBackupService() {
		this(Clock.systemDefaultZone(), new TenantEncryptionService());
	}

	TenantBackupService(Clock clock) {
		this(clock, new TenantEncryptionService());
	}

	TenantBackupService(Clock clock, TenantEncryptionService encryptionService) {
		this.clock = clock;
	...
	private void createEncryptedZipBackup(TenantSession session, Path backupFile) throws IOException {
		...
		try {
			...
			encryptionService.writeEncryptedContent(tempFile, session, outputStream -> {
			...
			encryptionService.moveAtomically(tempFile, backupFile);
