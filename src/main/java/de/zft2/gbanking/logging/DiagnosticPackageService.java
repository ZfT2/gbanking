...
public final class DiagnosticPackageService {
	...
	DiagnosticPackageService(Path logDirectory) {
	...
	private List<Path> findLogFiles() throws IOException {
		...
		try (var files = Files.list(logDirectory)) {
			return files.filter(path -> Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS)).filter(DiagnosticPackageService::isGbankingLog)
					.sorted(Comparator.comparing(DiagnosticPackageService::fileName)).toList();
		}
	}
