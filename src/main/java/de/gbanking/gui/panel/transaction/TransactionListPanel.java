...
public class TransactionListPanel extends AbstractFilterableTablePanel<Booking> {
	...
	private de.gbanking.db.dao.BankAccount resolveContextAccount(Booking selectedBooking) {
		de.gbanking.db.dao.BankAccount account = currentContextAccount();
		if (account != null) {
		...
		account = dbController.getById(de.gbanking.db.dao.BankAccount.class, selectedBooking.getAccountId());
		if (account != null) {
			parentPanel.getTransactionDetailPanel().setCurrentAccount(account);
		}
		return account;
	...
	private de.gbanking.db.dao.BankAccount currentContextAccount() {
