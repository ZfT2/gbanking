...
public class StatementsLogicRecipient extends StatementsLogicDefault<Recipient, Void> implements StatementsLogic<Recipient, Void> {
	...
	public StatementType getStatementTypeForInsertOrUpdate(Recipient recipient) {
		...
		Recipient recipientDb = getById(Recipient.class, executeSelectId(SqlStatements.SQL_SELECT_ID_RECIPIENT_BY_IBAN, recipient.getIban(), null));
		if (recipientDb != null) {
		...
		if (id > 0) {
			if (executeSelectId(SqlStatements.SQL_SELECT_RECIPIENT_BY_ID_IF_NOT_REFERENCED, String.valueOf(recipientDb.getId()), null) > 0) {
