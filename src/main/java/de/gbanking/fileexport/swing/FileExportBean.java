...
public abstract class FileExportBean extends BaseBean {
	...
	private static Logger log = LogManager.getLogger(FileExportBean.class);
	...
	private void updateWorkerState(long importedCount, long totalCount, int percentageStart, int percentageStop, String message, Object... param) {
		if (worker != null) {
			...
			int stop = progress > percentageStop ? progress = percentageStop : progress;
			progress = progress < percentageStart ? progress + percentageStart : stop;
