...
LEFT JOIN moneytransferProtocol mp ON mp.id = (
    SELECT protocol.id
    FROM moneytransferProtocol protocol
    WHERE protocol.moneytransfer_id = m.id AND protocol.bankOrderId IS NOT NULL
    ORDER BY protocol.timeStart DESC, protocol.id DESC
    LIMIT 1)
LEFT JOIN moneytransferProtocol mps ON mps.id = (
    SELECT protocol.id
    FROM moneytransferProtocol protocol
    WHERE protocol.moneytransfer_id = m.id AND protocol.sepaOrderStatus IS NOT NULL
    ORDER BY protocol.timeStart DESC, protocol.id DESC
    LIMIT 1)
WHERE 1 = 1;
...
${SQL_SELECT_ALL_MONEYTRANSFERS_BASE} AND m.moneytransferstatus = ?;

[SQL_SELECT_ALL_MONEYTRANSFERS_WITH_RECIPIENTS_BY_ACCOUNT_AND_STATE]
${SQL_SELECT_ALL_MONEYTRANSFERS_BASE} AND m.account_id = ? AND m.moneytransferstatus = ?;
...
SET account_id = ?, moneytransferType = ?, recipient_id = ?, purpose = ?, purposeCode = ?, endToEndId = ?, amount = ?, executionDate = ?, executionDay = ?, moneytransferStatus = ?, standingorderMode = ?, historyorder_id = ?, updatedAt = ?
WHERE id = ?;

[SQL_INSERT_MONEYTRANSFER_PROTOCOL]
INSERT INTO moneytransferProtocol (moneytransfer_id, moneytransferStatus, timeStart, timeFinish, bankOrderId, sepaOrderStatus, sepaCancellationCode, protocolText, updatedAt)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);

[SQL_SELECT_ALL_MONEYTRANSFER_PROTOCOLS]
SELECT id, moneytransfer_id, moneytransferStatus, timeStart, timeFinish, bankOrderId, sepaOrderStatus, sepaCancellationCode, protocolText, updatedAt
