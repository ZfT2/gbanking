...
SELECT m.id, m.account_id, m.moneytransferType, m.recipient_id, m.purpose, m.amount, m.executionDate, m.moneytransferStatus, m.standingorderMode, m.historyorder_id, m.updatedAt, r.id AS r_id, r.name, r.iban, r.bic, r.bank FROM moneytransfer m, recipient r WHERE m.recipient_id = r.id
...
INSERT INTO moneytransfer (account_id, moneytransferType, recipient_id, purpose, amount, executionDate, moneytransferStatus, standingorderMode, historyorder_id, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
...
INSERT INTO moneytransferProtocol (moneytransfer_id, moneytransferStatus, timeStart, timeFinish, protocolText, updatedAt) VALUES (?, ?, ?, ?, ?, ?);
;
