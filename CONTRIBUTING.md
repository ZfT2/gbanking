...
GitHub Actions runs a compile and test check after each push to a branch:

```
xvfb-run -a mvn -B clean test
```

The workflow waits 120 seconds before starting. If another commit is pushed to
the same branch during that window, the older run is canceled and only the newest
commit continues. The test step uses a virtual display for JavaFX and stops after
20 minutes if it does not complete.
